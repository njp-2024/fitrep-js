    ...
    if (rptNameInput) {
        rptNameInput.addEventListener('input', (e) => {
            ...
            if (currentName === "") {
                ...
                updateRealTimeAnalysis(); // <--- REVERTS TO SAVED STATE
                return;
            }
            ...
            // C. Auto-Populate (Only if not already dirty)
            ...
            // D. Update Everything
            updateAddButtonState();
            updateRealTimeAnalysis(); // <--- RUNS THE PROJECTION
        });
    }
...
function recalculateProfileStatsOLD() {
...
// --- LOGIC: RECALCULATE (COMMITS TO STATE) ---
function recalculateProfileStats() {
    // 1. Calculate based on the REAL saved list
    const finalProfile = calculateStatsFromList(reportsList);
...
// --- UI UPDATER: REPORTS LEDGER (Main & Sidebar) ---
function updateReportsLedger() {
    // 1. Identify Targets
    const mainTbody = document.getElementById('main-reports-list'); // The Big Table
    const sideTbody = document.getElementById('sb-reports-body');   // The Sidebar Table
    const finishBtn = document.getElementById('btn-finish-reports');

    // 2. Clear Existing Content (Safe check)
    if (mainTbody) mainTbody.innerHTML = "";
    if (sideTbody) sideTbody.innerHTML = "";

    // 3. Handle Empty State
    if (reportsList.length === 0) {
        const emptyMsg = `<tr><td colspan="4" class="text-muted fst-italic py-2">No reports yet.</td></tr>`;
        
        if (mainTbody) mainTbody.innerHTML = emptyMsg;
        if (sideTbody) sideTbody.innerHTML = emptyMsg;
        if (finishBtn) finishBtn.disabled = true;
        return;
    }

    // 4. Unlock Next Button
    if (finishBtn) finishBtn.disabled = false;

    // 5. Running Calculation & Row Generation
    let runningScore = originalProfile.avg * originalProfile.reports;
    let runningCount = originalProfile.reports;
    ...
    reportsList.forEach(report => {
        ...
        // B. Generate HTML String (Re-usable!)
        // Note: We use 'text-truncate' on the name so long names don't break the sidebar
        const rowHtml = `
            <tr>
        ...
        // C. Inject into Main Table (if it exists on this page)
        if (mainTbody) {
            // Main table has an extra 'Proc' column in HTML, so we might need a slightly different string
            // Or we just inject the specific columns. 
            // To keep it simple, let's inject a slightly adjusted row for the main table:
            const mainRow = `
                <tr>
            ...
            `;
            mainTbody.innerHTML += mainRow;
        }

        // D. Inject into Sidebar (Always exists)
        if (sideTbody) {
            sideTbody.innerHTML += rowHtml;
        }
    });
}

// --- HELPER: SCRAPE FORM DATA ---
function getFormReport() {
    const nameInput = document.getElementById('rpt-name');
    if (!nameInput) return null;

    const name = nameInput.value.trim();
    if (!name) return null; // Logic stops if no name

    // Scrape Scores
    const checkedRadios = document.querySelectorAll('#report-attributes-container input[type="radio"]:checked');
    // If user hasn't selected all buttons yet, we still want to calculate what we have so far
    // So we map what we find, treating missing ones as 0 (effectively ignoring them)
    const scores = Array.from(checkedRadios).map(r => parseInt(r.value, 10));

    // Return a temporary Report object
    return new Report(name, activeProfile.rank, scores);
}

// --- LOGIC: REAL-TIME PROJECTOR ---
function updateRealTimeAnalysisOLD() {
    // 1. Scrape the Form
    const tempReport = getFormReport();

    // 2. If form is invalid/empty, Revert to Saved State
    if (!tempReport) {
        recalculateProfileStats(); // Resets sidebar to reality
        return;
    }

    // 3. Create a "Projected" List (Copy of the real list)
    let projectedList = [...reportsList];

    // 4. Handle "Edit" vs "New" Logic
    const existingIndex = projectedList.findIndex(r => r.name.toLowerCase() === tempReport.name.toLowerCase());

    if (existingIndex !== -1) {
        // SCENARIO: User is editing an existing report
        // We swap the saved version with the live form version
        projectedList[existingIndex] = tempReport;
    } else {
        // SCENARIO: User is creating a new report
        // We add the form version to the list
        projectedList.push(tempReport);
    }

    // 5. Calculate Stats on the PROJECTED List
    // (This logic mirrors recalculateProfileStats but uses the temp list)
    let totalScore = originalProfile.avg * originalProfile.reports;
    let totalCount = originalProfile.reports;

    projectedList.forEach(rpt => {
        totalScore += rpt.average;
        totalCount++;
    });
    ...
    //const projectedAvg = totalCount > 0 ? (totalScore / totalCount) : 0;

    // 6. Create a Temporary Profile Object for the UI
    // We clone the active profile so we don't accidentally save these numbers yet
    //const tempProfile = activeProfile.clone();
    //tempProfile.avg = projectedAvg;
    //tempProfile.reports = totalCount;

    // Recalculate High/Low for the projection
    //tempProfile.high = originalProfile.high;
    //tempProfile.low = originalProfile.low;
    //projectedList.forEach(rpt => {
    //    if (rpt.average > tempProfile.high) tempProfile.high = rpt.average;
    //    if (rpt.average < tempProfile.low) tempProfile.low = rpt.average;
    //});

    // 7. Update UI
    updateSidebar(originalProfile, activeProfile);      // Updates the Top Card (Stats)
    updateActiveReportCard();        // Updates the Middle Card (Current Report)
    // We intentionally DO NOT update the Bottom Card (Ledger) until saved (Option A)
}

// --- LOGIC: REAL-TIME PREVIEW (NO COMMIT) ---
function updateRealTimeAnalysis() {
    // 1. Scrape Form
    const tempReport = getFormReport();

    // 2. If invalid, revert to reality
    if (!tempReport) {
        recalculateProfileStats();
        return;
    }

    // 3. Build Projected List
    let projectedList = [...reportsList]; // Copy existing
    const existingIndex = projectedList.findIndex(r => r.name.toLowerCase() === tempReport.name.toLowerCase());

    if (existingIndex !== -1) {
        projectedList[existingIndex] = tempReport; // Scenario: Edit
    } else {
        projectedList.push(tempReport); // Scenario: Add
    }

    // 4. Calculate based on PROJECTED list
    const tempProfile = calculateStatsFromList(projectedList);

    // 5. Update UI ONLY (Do not save to activeProfile)
    if (tempProfile) {
        updateSidebar(tempProfile);      // <--- THIS FIXES YOUR BUG
        updateActiveReportCard();
    }
}

// --- LOGIC: PURE CALCULATOR ---
// Returns a new UserProfile object based on the list provided
function calculateStatsFromList(targetList) {
    if (!originalProfile) return null;

    // 1. Start with Base Stats (Original Profile)
    let totalScore = originalProfile.avg * originalProfile.reports;
    let totalCount = originalProfile.reports;

    // 2. Add Mass from the Target List
    targetList.forEach(rpt => {
        totalScore += rpt.average;
        totalCount++;
    });

    // 3. Create Result Object (Clone original to get Rank/Name)
    const resultProfile = originalProfile.clone();
    ...
    // 5. Update High/Low
    // Reset to original bounds, then stretch if new reports exceed them
    resultProfile.high = originalProfile.high;
    resultProfile.low = originalProfile.low;

    targetList.forEach(rpt => {
        if (rpt.average > resultProfile.high) resultProfile.high = rpt.average;
        if (rpt.average < resultProfile.low) resultProfile.low = rpt.average;
    });
